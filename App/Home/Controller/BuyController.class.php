...
class BuyController extends Controller
{
	...
	{
		header('location:'.U('Agent/Index/index'));
	}
	...
	public function reservation()
	{
		...
		if(session('cart')['type'] == 'reservation'){
			if(IS_POST){
				...
				$order_goods['time'] = time();
				$order_goods_res = M('order_goods')->add($order_goods);
				$addgoods = $this->addgoods($order_res,0);//给用户添加商品
				if($order_res && $order_goods_res && $addgoods){
					...
					// session('cart',null);
					// header('location:'.U('pay').'?order='.urlencode($orderid).'&buy='.urlencode($buy));//加密后在编码(编码解决get特殊符号无法接收)
					 $this->success('预定成功！',U('Agent/Procurement/index'));
	...
	protected function addgoods($order_id,$status)
	{
		...
		$res = M('agent_goods')->add($datas);

		//改变订单状态
		$order_res = M('order')->where(array('order_id'=>$order_id))->save(array('shipping_status'=>1));
		if($res && $order_res){
			return 1;
		}else{
			return false;
